class UnbeatableAiPlayer
  ...
  def get_move(board)
    ...
    board.open_spaces.each do |space|
      ...
      score = negamax(board, false, opponent, 100, -100, 1)
      if score > top_score
        top_score = score
        move_index = space
      end
  ...
  def negamax(board, my_turn, opponent, alpha, beta, depth)
    ...
    board.open_spaces.each do |space|
      ...
      score = -negamax(board, !my_turn, opponent, -beta, -alpha, depth + 1)
      if score < alpha
        alpha = score
      end
      board.place_move(space, nil)
